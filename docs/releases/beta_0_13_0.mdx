...
Automatic RPC endpoint failover is now supported. When an RPC endpoint cannot be reached, Dedot automatically selects an alternative endpoint to establish the connection. Failover is handled internally by Dedot and applies to **Auto RPC** connections (random endpoint selection). **Light Client** connections using the smoldot client is also supported.
